...
namespace Backend_Dashboard.Core.Services
{
    public class AuthService : IAuthService
    {
        ...
        public async Task<GeneralServiceResponseDto> RegisterAsync(RegisterDto registerDto)
        {
            ...
            var createUserResults = await _userManager.CreateAsync(newUser, registerDto.Password);

            if (!createUserResults.Succeeded)
            {
                var errorString = "User Creation failed because: ";
                foreach (var error in createUserResults.Errors)
                {
        ...
        public async Task<GeneralServiceResponseDto> UpdateRoleAsync(ClaimsPrincipal User, UpdateRoleDto updateRoleDto)
        {
            ...
            if (User.IsInRole(StaticUserRoles.ADMIN))
            {
                ...
                {
                    //admin can change the role of everyone except the owner and admins
        ...
        {
            //Instead of this, We can Use Automapper packages.
